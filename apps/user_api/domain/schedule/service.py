...
# 특정 사용자의 모든 스케줄 조회하기
def get_schedules_by_user_id(db: Session, user_id: int) -> list[Schedule]:
    """
    특정 사용자의 모든 스케줄을 조회합니다.
    """
...
def check_and_run_due_schedules():
    ...
    for schedule in all_schedules:
        ...
        if iter.get_prev(datetime) == now.replace(second=0, microsecond=0): 
            ...
            print(f"실행 시간 도달. 스케줄 ID: {schedule.schedule_id}, 내용: {schedule.content}")
